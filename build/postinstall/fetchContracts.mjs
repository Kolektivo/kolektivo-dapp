...
export default async name => {
  ...
  try {
    ...
    switch (name) {
      case "monetary":
        ...
        const { stdout } = await exec(
          `git clone --depth 1 https://github.com/Kolektivo/kolektivo-monetary-contracts.git ${srcPath}`
        );
        console.log(stdout);
        break;
      case "governance":
        srcPath = "node_modules/kolektivo-governance-contracts";
        const { stdout1 } = await exec(
          `git clone --depth 1 https://github.com/Kolektivo/kolektivo-governance-contracts.git ${srcPath}`
        );
        console.log(stdout1);
        break;
      case "map":
        srcPath = "node_modules/kolektivo-map-contracts";
        const { stdout2 } = await exec(
          `git clone --depth 1 https://github.com/Kolektivo/kolektivo-map-contracts.git ${srcPath}`
        );
        console.log(stdout2);
        break;
    }
    ...
    await Promise.all(
      files.map(file => fs.copy(file, `${destPath}/${path.basename(file)}`))
    );
