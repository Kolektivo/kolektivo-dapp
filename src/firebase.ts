...
import { IEthereumService, IIpfsService, INumberService, Services, fromWei } from 'services';
...
const container = DI.createContainer()
  ...
  .register(Registration.instance(IEthereumService, {}))
  .register(Services)
  .register(Store)
  .register(
...
await Promise.all(
  periods.map(async (period): Promise<void> => {
    ...
    if (now >= lastSyncTime || !lastSync || true) {
