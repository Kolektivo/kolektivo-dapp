...
export class Assets implements ICustomElementViewModel {
  ...
  constructor(@I18N private readonly i18n: I18N, @ITreasuryStore private readonly treasuryStore: ITreasuryStore) {
    this.columns = [
      {
        headerText: this.i18n.tr('navigation.treasury.overview.assets.assets-tab.grid-headers.token'),
        field: 'token',
        width: '1fr',
        template: '<avatar-text name.bind="token.name" src.bind="token.logoURI"></avatar-text>',
      },
      {
        headerText: this.i18n.tr('navigation.treasury.overview.assets.assets-tab.grid-headers.price'),
        field: 'price',
        width: '1fr',
        template: '<span>${token.price | currency}</span>',
      },
      {
        headerText: this.i18n.tr('navigation.treasury.overview.assets.assets-tab.grid-headers.quantity'),
        field: 'quantity',
        width: '1fr',
        template: '<formatted-number show-tooltip commas value="${quantity | ethwei}">',
      },
      {
        headerText: this.i18n.tr('navigation.treasury.overview.assets.assets-tab.grid-headers.total'),
        field: 'totalValue',
        width: '1fr',
