...
describe('contracts-service.ts', () => {
  it('transfers a token', async () => {
    ...
    const tx = await token.transferFrom(testAccount1, testAccount2, transferAmount);
    ...
    expect(endingBalanceAccount1.sub(startingBalanceAccount1)).equals(transferAmount);
    expect(startingBalanceAccount2.sub(endingBalanceAccount2)).equals(transferAmount);
  });
});
