...
export const resolver = (container: IContainer) => {
  const configuration = container.get(IConfiguration);
    ...
    providerOptions: {
      'custom-kolektivo': {
        ...
        options: {
          ...
          rpc: {
            [configuration.chainId]: configuration.chainUrl,
          },
        },
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        connector: (walletConnectProvider: any, opts?: IWalletConnectConnectorOptions) =>
          ConnectToWalletConnect(walletConnectProvider, opts, configuration.chainId),
      },
